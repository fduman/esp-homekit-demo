...
bool am2320_i2c_informUser(const QueueHandle_t *resultQueue, am2320_temp_t temp, am2320_humid_t humid)
{
    printf("Temperature: %f, Humidity: %f\n", (float)temp, (float)humid);
    homekit_characteristic_notify(&temperature, HOMEKIT_FLOAT(temp));
    homekit_characteristic_notify(&humidity, HOMEKIT_FLOAT(humid));
    return true;
}
...
homekit_accessory_t *accessories[] = {
    HOMEKIT_ACCESSORY(.id = 1, .category = homekit_accessory_category_thermostat, .services = (homekit_service_t *[]){HOMEKIT_SERVICE(ACCESSORY_INFORMATION, .characteristics = (homekit_characteristic_t *[]){HOMEKIT_CHARACTERISTIC(NAME, "Temperature/Humidity Sensor"), HOMEKIT_CHARACTERISTIC(MANUFACTURER, "Furkan Duman"), HOMEKIT_CHARACTERISTIC(SERIAL_NUMBER, "0000002"), HOMEKIT_CHARACTERISTIC(MODEL, "Mini"), HOMEKIT_CHARACTERISTIC(FIRMWARE_REVISION, "2.0"), HOMEKIT_CHARACTERISTIC(IDENTIFY, temperature_sensor_identify), NULL}), HOMEKIT_SERVICE(TEMPERATURE_SENSOR, .primary = true, .characteristics = (homekit_characteristic_t *[]){HOMEKIT_CHARACTERISTIC(NAME, "Temperature Sensor"), &temperature, NULL}), HOMEKIT_SERVICE(HUMIDITY_SENSOR, .characteristics = (homekit_characteristic_t *[]){HOMEKIT_CHARACTERISTIC(NAME, "Humidity Sensor"), &humidity, NULL}), NULL}),
    NULL};
